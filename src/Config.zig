    ...
    while (it.next()) |raw_line| {
        ...
        if (std.mem.eql(u8, key, "theme")) {
            ...
            renderer = std.meta.stringToEnum(RendererBackend, value) orelse {
                std.log.warn("config: {s}:{}: invalid renderer '{s}' (expect d3d11, d3d12, opengl, pure-opengl, or native-vulkan)", .{ source_name, line_no, value });
                continue;
            };
...
test "research renderers require exact explicit names" {
    ...
    inline for (.{ RendererBackend.d3d12, RendererBackend.opengl, RendererBackend.@"pure-opengl", RendererBackend.@"native-vulkan" }) |backend| {
